...
class LSPTool:
    ...
    def _get_python_diagnostics(self, file_path: str, content: str) -> str:
        ...
        # wanted to try pyflakes, but seemed to sometimes work will look into later
        try:
                ...
                if result.stdout:
                    ...
                    for issue in pyflakes_issues:
                        if issue.strip():
                            ...
                            diagnostics.append(f"⚠️  Pyflakes: {clean_issue}")
    ...
    def _get_js_ts_diagnostics(self, file_path: str, content: str) -> str:
        ...
        for i, line in enumerate(lines, 1):
            ...
            if 'console.log(' in line and not line_stripped.startswith('//'):
                warning_count += 1
                diagnostics.append(f"⚠️  Warning (line {i}): console.log() found")
            
            if line_stripped.endswith(';') and line_stripped.count(';') > 1:
                warning_count += 1
                diagnostics.append(f"⚠️  Warning (line {i}): Multiple statements on one line")
            
            if len(line) > 120:
                warning_count += 1
                diagnostics.append(f"⚠️  Warning (line {i}): Line too long ({len(line)} chars)")
            ...
            if (line_stripped and 
                ...
                warning_count += 1
                diagnostics.append(f"⚠️  Warning (line {i}): Missing semicolon?")
    ...
    def _get_generic_diagnostics(self, file_path: str, content: str) -> str:
        ...
        for i, line in enumerate(lines, 1):
            if len(line) > 200:
                warning_count += 1
                diagnostics.append(f"⚠️  Warning (line {i}): Very long line ({len(line)} chars)")
            
            if line.rstrip() != line:
                warning_count += 1
                diagnostics.append(f"⚠️  Warning (line {i}): Trailing whitespace")
        
        # File-level checks
        if not content.strip():
            warning_count += 1
            diagnostics.append("⚠️  Warning: File is empty")
        
        if not content.endswith('\n'):
            warning_count += 1
            diagnostics.append("⚠️  Warning: File doesn't end with newline")
